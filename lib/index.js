...
class FontminPlugin {
  ...
  onAdditionalAssets(compilation) {
    ...
    const appendTimestamp = this._options.appendTimestamp
        ...
        .then(files => {
          files.forEach(file => {
            if (file.buffer.length > file.minified.length) {
              let assetName = file.asset;
              if ( appendTimestamp ) {
                assetName = assetName.split('.').join(`-${timestamp}.`)
              }
              compilation.assets[ assetName ] = new RawSource(file.minified)
            }
          })
        })
    }, Promise.resolve())
