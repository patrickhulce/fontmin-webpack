      ...
      return prev
        ...
        .then(files => {
          files.forEach(file => {
            ...
            if ( appendHash ) {
              let fileHash = crypto.createHash( 'md5' ).update( file.minified ).digest( 'hex' )
              let oldAssetName = file.asset
              let newAssetName = oldAssetName.split('.').join(`-${fileHash}.`)
              compilation.assets[ newAssetName ] = compilation.assets[ oldAssetName ];
              delete compilation.assets[ oldAssetName ];
              this.hashifyFontReferences(oldAssetName, newAssetName, compilation.assets)
            }              
          })
        })
    }, Promise.resolve())
